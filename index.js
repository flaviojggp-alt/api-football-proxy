...
// ── CACHE A NIVEL PROXY (5-10 min TTL) ─────────────────────────────────────
// Todas las llamadas a API-Football pasan por af(), así que cachear aquí
// cubre automáticamente cualquier combinación de teamId/opponentId/leagueId
// sin tener que tocar cada endpoint individualmente. Los datos de stats,
// h2h, xg, lesiones, etc. no cambian minuto a minuto, así que un TTL corto
// reduce drásticamente las llamadas repetidas cuando el usuario analiza el
// mismo partido varias veces o varios partidos comparten equipos.
const _afCache = new Map(); // path completo (con querystring) -> { data, ts }
const AF_CACHE_TTL_MS = 8 * 60 * 1000; // 8 minutos

// Limpieza periódica para que el Map no crezca sin límite en procesos de larga duración
setInterval(() => {
  const now = Date.now();
  for (const [key, entry] of _afCache) {
...
// poder exponerlos en '/' y para loguear una advertencia cuando queda poco
// margen (evita enterarnos del límite solo cuando ya empiezan a fallar los
// requests).
...
const RATE_LIMIT_WARN_THRESHOLD = 200; // avisar cuando quedan <=200 requests del día

async function af(path) {
  const cached = _afCache.get(path);
  if (cached && (Date.now() - cached.ts) < AF_CACHE_TTL_MS) {
    return cached.data;
  }
  ...
  // Headers de rate limit (nombres pueden variar mayúsc/minúsc según el runtime)
  const dailyRemaining = r.headers.get('x-ratelimit-requests-remaining');
  const dailyLimit = r.headers.get('x-ratelimit-requests-limit');
  if (dailyRemaining !== null) {
    rateLimitStatus.dailyRemaining = parseInt(dailyRemaining, 10);
    rateLimitStatus.dailyLimit = dailyLimit ? parseInt(dailyLimit, 10) : rateLimitStatus.dailyLimit;
    rateLimitStatus.lastChecked = new Date().toISOString();
    if (rateLimitStatus.dailyRemaining <= RATE_LIMIT_WARN_THRESHOLD) {
      console.warn(`⚠️  API-Football: quedan solo ${rateLimitStatus.dailyRemaining} requests hoy (límite diario ${rateLimitStatus.dailyLimit})`);
    }
  }

  const data = await r.json();
  _afCache.set(path, { data, ts: Date.now() });
  return data;
}
